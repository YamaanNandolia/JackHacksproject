...
async function handleJobWorkspace(job: JacCoderJob): Promise<void> {
...
async function ensureActiveFileContext(entryPoint: string): Promise<boolean> {
  const folders = vscode.workspace.workspaceFolders ?? [];
  if (folders.length === 0) {
    log("No workspace folder available to open an active file.");
    return false;
  }
  ...
  const targetPath = preferredTargets.find((candidate) => fs.existsSync(candidate));
  if (!targetPath) {
    log(
      `Active file context skipped: neither PROMPT.md nor entry point was found in ${workspaceRoot}`
    );
    return false;
  }
  ...
  try {
    ...
    log(`Opened active file for JacCoder context: ${targetPath}`);
    return true;
  } catch (err) {
    log(`ERROR: Could not open active file for JacCoder context: ${String(err)}`);
    return false;
  }
}

function selectAllInActiveEditor(): boolean {
  const editor = vscode.window.activeTextEditor;
  if (!editor) {
    log("No active editor available for selection.");
    return false;
  }
  ...
  log("Selected full contents of the active file.");
  return true;
}
...
async function tryTriggerJacCoder(
  ...
  if (toTry.length === 0) {
    ...
    for (const cmd of toTry) {
      ...
      if (succeeded) {
        const promptInjectionResult = await ensurePromptInjected(found, cmd, prompt);
        if (promptInjectionResult === "done") {
        ...
        if (promptInjectionResult === "fallback") {
          log(`  Prompt injection deferred after "${cmd}" — switching to AppleScript.`);
          await runAppleScriptFallback(found, job, true);
          return;
        }

        log(`  Prompt injection did not complete after "${cmd}" — continuing.`);
      }
    }
...
async function ensurePromptInjected(
  ...
  commandId: string,
  prompt: string
): Promise<"done" | "fallback" | "continue"> {
  if (!prompt.trim()) {
    log("  No prompt text provided — treating open command as sufficient.");
    return "done";
  }

  const lowerId = found.ext.id.toLowerCase();
  const lowerCommand = commandId.toLowerCase();

  // JacCoder flow:
  //  1. Open PROMPT.md in editor and select all text
  //  2. Call jaccoder.addToChat  → attaches the file as context
  //  3. Call jaccoder.focusChat  → moves keyboard focus INTO the chat input box
  //  4. Write the activation message to the clipboard
  //  5. Paste (Cmd+V) + Return via AppleScript — no Tab needed because
  //     jaccoder.focusChat already placed the cursor in the input
  if (lowerId.includes("jaccoder") || lowerCommand.startsWith("jaccoder.")) {
    log("  JacCoder session opened; waiting 1.5s for panel to settle…");
    await delay(1500);

    // ── Step 1: open PROMPT.md and select its contents ───────────────────
    const opened = await ensureActiveFileContext("PROMPT.md");
    if (!opened) {
      log("  WARN: Could not open PROMPT.md — skipping addToChat.");
    } else {
      log("  PROMPT.md opened; waiting 1s before selecting text.");
      await delay(1000);
      selectAllInActiveEditor();

      // ── Step 2: attach PROMPT.md to the chat ─────────────────────────
      log("  PROMPT.md selected; waiting 1s before calling addToChat.");
      await delay(1000);
      for (const injectCmd of ["jaccoder.addToChat", "jaccoder.attachToChat"]) {
        if (!found.commands.includes(injectCmd)) {
          continue;
        }
        log(`  → Attaching PROMPT.md via: ${injectCmd}`);
        try {
          await vscode.commands.executeCommand(injectCmd);
          log(`  ✓ PROMPT.md attached via: ${injectCmd}`);
        } catch (err) {
          log(`    ✗ ${injectCmd} failed: ${String(err)}`);
        }
        break; // only need one attach command
      }
    }

    // ── Step 3: focus the JacCoder chat input via its dedicated command ──
    log("  Calling jaccoder.focusChat to move focus into the chat input…");
    await delay(800);
    try {
      await vscode.commands.executeCommand("jaccoder.focusChat");
      log("  ✓ jaccoder.focusChat executed — chat input should now have focus.");
    } catch (err) {
      log(`  WARN: jaccoder.focusChat failed: ${String(err)} — will try anyway.`);
    }

    // ── Step 4: write activation text to the system clipboard ────────────
    const activationText =
      "Build this application as described in the attached prompt.";
    await vscode.env.clipboard.writeText(activationText);
    log("  ✓ Activation text written to clipboard.");

    // ── Step 5: signal the external launcher watcher to paste + submit ───
    // We cannot run osascript from inside the VS Code extension host —
    // macOS blocks child processes of VS Code from controlling other apps
    // via Accessibility.  Instead we write a trigger file that the launcher
    // watcher (running outside VS Code) detects and acts on.
    await delay(400);
    const workspaceName =
      vscode.workspace.workspaceFolders?.[0]?.name ?? "jaccoder";
    const triggerPath = path.join(os.tmpdir(), `jaccoder_paste_${workspaceName}.trigger`);
    try {
      fs.writeFileSync(triggerPath, activationText, "utf8");
      log(`  ✓ Paste trigger written: ${triggerPath}`);
      log("  External launcher watcher will now paste and submit.");
    } catch (writeErr) {
      log(`  WARN: Could not write trigger file: ${String(writeErr)}`);
      log("  Manual action: click the JacCoder input, paste (Cmd+V), and press Enter.");
    }

    return "done";
  }

  // For other extensions, preserve existing behavior.
  return "done";
}
...
async function runAppleScriptFallback(
  ...
  const paletteQuery = derivePaletteQuery(found, preferExistingSession);
  const postOpenDelaySeconds = preferExistingSession ? "5" : "2";
  log(`Palette query    : ${paletteQuery}`);
  log(`Post-open delay : ${postOpenDelaySeconds}s`);

  // ── Run osascript ────────────────────────────────────────────────────────
  log("Invoking osascript…");
  log(
    `  osascript "${scriptPath}" "${promptFilePath}" "${paletteQuery}" "${postOpenDelaySeconds}"`
  );

  return new Promise((resolve) => {
    execFile(
      "osascript",
      [scriptPath, promptFilePath, paletteQuery, postOpenDelaySeconds],
      { timeout: 30_000 },
      (err: Error | null, stdout: string, stderr: string) => {
        if (stderr) {
          // AppleScript log() writes to stderr — surface every line
          stderr.split("\n").filter(Boolean).forEach((line: string) => log(`  [AS] ${line}`));
        }
        if (stdout.trim()) {
...
function resolveAppleScriptPath(): string | null {
  const roots = [ctxExtensionPath];

  try {
    const realExtensionPath = fs.realpathSync(ctxExtensionPath);
    if (!roots.includes(realExtensionPath)) {
      roots.push(realExtensionPath);
    }
  } catch (err) {
    log(`WARN: Could not resolve real extension path: ${String(err)}`);
  }
...
function derivePaletteQuery(
  ...
  for (const [keyword, queries] of Object.entries(PALETTE_QUERY_CANDIDATES)) {
    if (id.includes(keyword)) {
      const chosen = preferExistingSession
        ? queries.find((query) => query.toLowerCase().includes("open")) ?? queries[0]
        : queries[0];
